...
#include "generated_tcl/search.inc"
    ...
    LeTerminalId le_terminal_by_name(LeHandle *handle, const char *name)
    {
        ...
        // separate "GUI current view" tracker (only ever moved as a side
        // effect of selecting a Design, e.g. le_set_current_design_by_id
        // - see those functions' own handle->scene.set_current_abstract
        // calls), independent by design from the has_current_access
        // surface get_terminals' own default scope and every other
        // resolve_X_id already derives from. Terminal is the one class
        // with a hand-written friendly-id resolver (unique_per_parent
        // means it can't use the generated by-name lookup pair - see
        // Field.unique_per_parent's own docstring), so it's the one place
        // this distinction was easy to get wrong: a script that builds an
        // Abstract from scratch and calls set_current_abstract directly
        // (no Design to open_design into) never touches Scene at all,
        // leaving resolve_terminal_id unable to find any Terminal it just
        // created even though get_terminals (same "current" concept)
